...
First, resolve any potential merge conflicts with master on the release candidate branch.

1. `git checkout master`
2. `git pull origin master`
3. `git checkout [release candidate branch]`
4. `git fetch origin [release candidate branch]`
...
8. Add the version change, commit, and push to `[release candidate branch]`.
9. Issue a pull request for merging the `[release candidate branch]` branch into the `master` branch.
10. Await approval, and then merge. _Note: do NOT use the "squash and merge" method; it will make subsequent merges more difficult. Instead, use the "Create a merge commit" option._

**In case of merge conflicts when merging [release candidate branch] into master, something went wrong. Investigate what happened thoroughly before continuing.**
...
5. Notify the team about the release. Delete the [release candidate branch].
6. Deploy onto (a) portal(s) for beta testing. Beta testers should have access to the release docs (i.e., they should already exist), so that feedback can be provided.
