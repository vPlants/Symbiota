<?php
/*
------------------
Language: Français
Date Translated: 2023-12-19
Translated by: Google Translate
------------------
*/
$LANG['CROWDSOURCING_ADMIN'] = 'Administración de Crowdsourcing';
$LANG['CROWDSOURCE_EXPLAIN'] = 'Ce module peut être utilisé pour soumettre et gérer des enregistrements pour la saisie de données par le
			grand public. Pour plus d\'informations, consultez le
			<a href="https://docs.symbiota.org/docs/Collection_Manager_Guide/Crowdsourcing/adding_crowdsourcing" target="_blank">
			Documentation Symbiota sur le crowdsourcing</a>';
$LANG['EDIT_PROJECT'] = 'Modifier le projet';
$LANG['GENERAL_INSTRUCTIONS'] = 'Instructions générales';
$LANG['TRAINING_URL'] = 'URL de formation';
$LANG['EDIT_CROWD_PROJ'] = 'Modifier le projet Crowdsource';
$LANG['TOTAL_COUNTS'] = 'Comptes totaux';
$LANG['RECS_IN_QUEUE'] = 'Enregistrements en file d\'attente';
$LANG['DEL_UNPROCESSED'] = 'Supprimer tous les enregistrements non traités de la file d\'attente';
$LANG['PENDING_APPROVAL'] = 'En attente d\'approbation';
$LANG['CLOSED_APPROVED'] = 'Fermé (Approuvé)';
$LANG['AVAILABLE_TO_ADD'] = 'Disponible pour ajouter';
$LANG['ADD_TO_QUEUE'] = 'Ajouter à la file d\'attente';
$LANG['CRITERIA'] = 'Critères';
$LANG['FAMILLE'] = 'Famille';
$LANG['GENUS_SP'] = 'Genre/Espèce';
$LANG['PAYS'] = 'Pays';
$LANG['STATE_PROV'] = 'État/Province';
$LANG['RECORD_LIMIT'] = 'Limite d\'enregistrement';
$LANG['VOLUNTEERS'] = 'Bénévoles';
$LANG['USER'] = 'Utilisateur';
$LANG['SCORE'] = 'Score';
$LANG['PENDING_REVIEW'] = 'En attente de révision';
$LANG['APPROVED'] = 'Approuvées';
$LANG['REVIEW'] = 'Révision';
$LANG['NO_RECS_PROCESSED'] = 'Aucun enregistrement traité';
$LANG['APPROVED_EDITORS'] = 'Éditeurs approuvés';
$LANG['SCORE_BOARD'] = 'Tableau des scores';
$LANG['NO_COLLID'] = 'ERREUR : identifiant de collection non fourni';

?>