<?php
/*
------------------
Language: Português (Portuguese)
Translated by: Google Translate (2024-02-16)
------------------
*/

$LANG['NAV_HOME'] = 'Início';
$LANG['CHECKLIST_ADMIN'] = 'Administração da Inventários Taxonômicos';
$LANG['MANAGE_VOUCHERS'] = 'Gerenciar Vouchers Vinculados';
$LANG['EDIT_LIST'] = 'Editar Lista de Espécies';
$LANG['OPEN_KEY'] = 'Abrir Chave Symbiota';
$LANG['GAMES'] = 'Jogos';
$LANG['NAMEGAME'] = 'Nomear Jogo';
$LANG['FLASH'] = 'Teste de Cartão Flash';
$LANG['SENSITIVE_SPECIES'] = 'Inventário Taxonômico de Espécies Sensíveis para';
$LANG['NUMBER_PENDING'] = 'Número de espécimes pendentes de proteção';
$LANG['PROTECT_LOCALITY'] = 'Proteger Localidades';
$LANG['CHECK_STATUS'] = 'Verificar Status';
$LANG['EXCLUSION_LIST'] = 'Lista de Espécies Excluídas para';
$LANG['INCLUDE_TAXA'] = 'Inclui táxons das seguintes listas de verificação secundárias';
$LANG['TAXA_EXCLUDED'] = 'Taxa explicitamente excluída';
$LANG['AUTORS'] = 'Autores';
$LANG['MOREDETAILS'] = 'mais detalhes';
$LANG['LESSDETAILS'] = 'menos detalhes';
$LANG['CITATION'] = 'Citação';
$LANG['OPTIONS'] = 'Opções';
$LANG['SEARCH'] = 'Pesquisar';
$LANG['DISPLAY_SYNONYMS'] = 'Exibir Sinônimos';
$LANG['COMMON'] = 'Nomes Comuns';
$LANG['SYNON'] = 'Sinônimos';
$LANG['FILTER'] = 'Filtro Taxonômico';
$LANG['OGCHECK'] = 'Inventário Taxonômico Original';
$LANG['NOTESVOUC'] = 'Notas e Vouchers';
$LANG['TAXONAUTHOR'] = 'Autores do Táxon';
$LANG['TAXONABC'] = 'Mostrar Táxons em Ordem Alfabética';
$LANG['SHOWSUBGENERA'] = 'Mostrar classificação subgenérica dentro do nome científico';
$LANG['ACTIVATEKEY'] = 'Ativar chave de identificação';
$LANG['BUILD_LIST'] = 'Construir Lista';
$LANG['DOWNLOAD_CHECKLIST'] = 'Baixar Inventário Taxonômico';
$LANG['PRINT_BROWSER'] = 'Imprimir no Navegador';
$LANG['EXPORT_DOCX'] = 'Exportar para DOCX';
$LANG['DISPLAYIMAGES'] = 'Exibir como Imagens';
$LANG['DISPLAYVOUCHERIMAGES'] = 'Use imagens de vouchers como imagem preferida';
$LANG['SYNONYMS'] = 'Sinônimos';
$LANG['NEWSPECIES'] = 'Adicionar Espécie à Lista';
$LANG['TAXON'] = 'Táxon';
$LANG['MORPHOSPECIES'] = 'Sufixo de Morfoespécie';
$LANG['FAMILYOVERRIDE'] = 'Substituição de Família';
$LANG['FAMILYOVERRIDE_DESCR'] = 'Para substituir a família atual';
$LANG['HABITAT'] = 'Habitat';
$LANG['ABUNDANCE'] = 'Abundância';
$LANG['NOTES'] = 'Notas';
$LANG['INTNOTES'] = 'Notas Internas';
$LANG['ADMIN_ONLY'] = 'Exibido apenas para administradores';
$LANG['SOURCE'] = 'Fonte';
$LANG['ADD_SPECIES'] = 'Adicionar Espécie à Lista';
$LANG['VOUCHER_MAPPING'] = 'Mapeamento de vouchers';
$LANG['BATCH_LOAD_SPREADSHEET'] = 'Planilha de Upload em Lote';
$LANG['VOUCHERS_SIMPLE_MAP'] = 'Exibir Vouchers no Mapa Simples';
$LANG['SIMPLE_MAP'] = 'Mapa Simples';
$LANG['VOUCHERS_DYNAMIC_MAP'] = 'Exibir Vouchers no Mapa Dinâmico';
$LANG['DYNAMIC_MAP'] = 'Mapa Dinâmico';
$LANG['OCCUR_DYNAMIC_MAP'] = 'Exibir Ocorrências no Mapa Dinâmico';
$LANG['FAMILIES'] = 'Famílias';
$LANG['GENERA'] = 'Gênero';
$LANG['SPECIES'] = 'Espécie';
$LANG['TOTAL_TAXA'] = 'Taxas Totais';
$LANG['DETAILS'] = 'detalhes';
$LANG['IMAGE'] = 'Imagem';
$LANG['NOTY'] = 'ainda não';
$LANG['AVAIL'] = 'disponível';
$LANG['EDIT_DETAILS'] = 'editar detalhes';
$LANG['VIEW_RELATED'] = 'Ver amostras relacionadas';
$LANG['LINKTOINAT'] = 'Link para observação iNaturalist';
$LANG['MORE'] = 'mais';
$LANG['LESS'] = 'menos';
$LANG['NOTAXA'] = 'Nenhum táxon encontrado';
$LANG['CHECKNULL'] = 'ERRO: identificador de lista de verificação inválido';
$LANG['IS_PRIVATE'] = 'Checklist definida como privada';
$LANG['DISPLAYNEXT'] = 'Exibir próximo';
$LANG['TAXA'] = 'taxa';
$LANG['LOCALITY'] = 'Localidade';
$LANG['ABSTRACT'] = 'Resumo';
$LANG['COMMENTS'] = 'Comentários';
$LANG['PAGE'] = 'Página';
$LANG['OF'] = 'de';
$LANG['CHECKLIST'] = 'Inventário Taxonômico';
$LANG['SEARCHSYNONYMS'] = "Pesquisar Sinônimos";
$LANG['TAXONFILTER'] = 'Filtro de Táxon';
$LANG['IMG_CHECKLIST_ADMIN'] = 'Botão de Administração da Inventário Taxonômico';
$LANG['IMG_MANAGE_VOUCHERS'] = 'Botão Gerenciar Vouchers Vinculados';
$LANG['IMG_EDIT_LIST'] = 'Botão Editar Lista de Espécies';
$LANG['IMG_OPEN_KEY'] = 'Botão de Abertura da Chave Symbiota';
$LANG['IMG_VOUCHERS_SIMPLE_MAP'] = 'Mostrar Vouchers no Mapa Simples';
$LANG['IMG_VOUCHERS_DYNAMIC_MAP'] = 'Mostrar Vouchers no Mapa Dinâmico';
$LANG['IMG_OCCUR_DYNAMIC_MAP'] = 'Mostrar todas as ocorrências no mapa dinâmico';
$LANG['EDIT_DETAILS'] = 'Botão Editar Detalhes';
$LANG['SEARCH_COMMON'] = 'Pesquisar Nomes Comuns';
$LANG['IMG_DWNL_LIST'] = 'Lista de Downloads';
$LANG['IMG_PRINT_LIST'] = 'Imprimir Lista';
$LANG['IMG_DOCX_EXPORT'] = 'Exportar para DOCX';
$LANG['SKIP_LINK'] = 'Pular para Lista de Táxons';
$LANG['IMAGE_SRC'] = 'Fonte da imagem';
$LANG['ALL_IMG'] = 'todas as imagens';
$LANG['LINKED_IMG'] = 'somente imagens de vouchers vinculados';

//de lista de verificação dinâmica.php
$LANG['ERROR_GEN_CHECK'] = 'ERRO ao gerar checklist';

?>
