<?php
/*
------------------
Language: Français (French)
------------------
*/


$LANG['NAV_HOME'] = 'Accueil';
$LANG['CHECKLIST_ADMIN'] = 'Administration d\'Liste';
$LANG['MANAGE_VOUCHERS'] = 'Gérer échantillons Liés';
$LANG['EDIT_LIST'] = 'Modifier Liste des Espéces';
$LANG['OPEN_KEY'] = 'Ouvrir Clé de Symbiota';
$LANG['GAMES'] = 'Jeux';
$LANG['NAMEGAME'] = 'Jeu de Nom';
$LANG['FLASH'] = 'Flashcard Quiz';
$LANG['SENSITIVE_SPECIES'] = 'Liste des espé sensibles pour';
$LANG['NUMBER_PENDING'] = 'Nombre de spécimens en attente de protection';
$LANG['PROTECT_LOCALITY'] = 'Proté Localités';
$LANG['CHECK_STATUS'] = 'Vérifer l\'État';
$LANG['EXCLUSION_LIST'] = 'Liste des Esp&egrave;ces d\'Éxclusion pour';
$LANG['INCLUDE_TAXA'] = 'Comprend taxons de liste d\'enfant suivant';
$LANG['TAXA_EXCLUDED'] = 'Taxa Explicitement Exclu';
$LANG['AUTHORS'] = 'Auteurs';
$LANG['MOREDETAILS'] = 'plus de détails';
$LANG['LESSDETAILS'] = 'moins de détails';
$LANG['CITATION'] = 'Citation';
$LANG['OPTIONS'] = 'Options';
$LANG['SEARCH'] = 'Rechercher';
$LANG['DISPLAY_SYNONYMS'] = 'Afficher Synonymes';
$LANG['COMMON'] = 'Noms Communs';
$LANG['SYNON'] = 'Synonymes';
$LANG['FILTER'] = 'Filtre Taxonomique';
$LANG['OGCHECK'] = 'Liste Originale';
$LANG['NOTESVOUC'] = 'Notes &amp; Échantillons';
$LANG['TAXONAUTHOR'] = 'Auteurs de Taxon';
$LANG['TAXONABC'] = 'Afficher Taxons par ordre Alphabétique';
$LANG['SHOWSUBGENERA'] = 'Afficher le classement sous-générique dans le nom scientifique';
$LANG['ACTIVATEKEY'] = 'Activer Clé d\'Identification';
$LANG['BUILD_LIST'] = 'Construire Liste';
$LANG['DOWNLOAD_CHECKLIST'] = 'Télécharger Liste';
$LANG['PRINT_BROWSER'] = 'Imprimer dans Navigateur';
$LANG['EXPORT_DOCX'] = 'Exporter vers DOCX';
$LANG['DISPLAYIMAGES'] = 'Afficher comme Images';
$LANG['DISPLAYVOUCHERIMAGES'] = 'Utiliser les images des Échantillons comme image préférée';
$LANG['SYNONYMS'] = 'Synonymes';
$LANG['NEWSPECIES'] = 'Ajouter Espéce á Liste';
$LANG['TAXON'] = 'Taxon';
$LANG['MORPHOSPECIES'] = 'Suffixe d\'Morphoesp&eacutece';
$LANG['FAMILYOVERRIDE'] = 'Remplacement de Famille';
$LANG['FAMILYOVERRIDE_DESCR'] = 'Pour remplacer la famille actuelle';
$LANG['HABITAT'] = 'Habitat';
$LANG['ABUNDANCE'] = 'Abondance';
$LANG['NOTES'] = 'Notes';
$LANG['INTNOTES'] = 'Notes Internes';
$LANG['ADMIN_ONLY'] = 'Affiché aux Administrateurs Seulement';
$LANG['SOURCE'] = 'Source';
$LANG['ADD_SPECIES'] = 'Ajouter Espéce á Liste';
$LANG['VOUCHER_MAPPING'] = 'Cartographie des Bons';
$LANG['BATCH_LOAD_SPREADSHEET'] = 'Lot télécharger Tableur';
$LANG['VOUCHERS_SIMPLE_MAP'] = 'Afficher Échantillons dans Carte Simple';
$LANG['SIMPLE_MAP'] = 'Carte Simple';
$LANG['VOUCHERS_DYNAMIC_MAP'] = 'Afficher Échantillons dans Carte Dynamique';
$LANG['DYNAMIC_MAP'] = 'Carte Dynamique';
$LANG['OCCUR_DYNAMIC_MAP'] = 'Afficher Toutes Occurrences dans Carte Dynamique';
$LANG['FAMILIES'] = 'Familles';
$LANG['GENERA'] = 'Genres';
$LANG['SPECIES'] = 'Esp&egrave;ces';
$LANG['TOTAL_TAXA'] = 'Taxons au Total';
$LANG['VOUCHER_COUNT'] = 'Échantillons Liés';
$LANG['DETAILS'] = 'détails';
$LANG['IMAGE'] = 'Image';
$LANG['NOTY'] = 'pas encore';
$LANG['AVAIL'] = 'disponible';
$LANG['EDIT_DETAILS'] = 'Modifier Détails';
$LANG['VIEW_RELATED'] = 'Voir Spécimens Associeés';
$LANG['LINKTOINAT'] = "Lien vers l'observation iNaturalist";
$LANG['MORE'] = 'plus';
$LANG['LESS'] = 'moins';
$LANG['NOTAXA'] = 'Aucun Taxa Trouvés';
$LANG['CHECKNULL'] = 'ERREUR: identifiant de liste invalide';
$LANG['IS_PRIVATE'] = 'La liste de contrôle est définie comme privée';
$LANG['DISPLAYNEXT'] = 'Afficher Prochain';
$LANG['TAXA'] = 'taxa';
$LANG['LOCALITY'] = 'Localité';
$LANG['ABSTRACT'] = 'Résumé';
$LANG['COMMENTS'] = 'Commentaires';
$LANG['PAGE'] = 'Page';
$LANG['OF'] = 'des';
$LANG['CHECKLIST'] = 'Liste';
$LANG['SEARCHSYNONYMS'] = "Rechercher des synonymes";
$LANG['TAXONFILTER'] = 'Filtre de Taxon';
$LANG['IMG_CHECKLIST_ADMIN'] = 'Bouton d\'administration de la liste de contrôle';
$LANG['IMG_MANAGE_VOUCHERS'] = 'Bouton Gérer les bons liés';
$LANG['IMG_EDIT_LIST'] = 'Bouton Modifier la liste des espèces';
$LANG['IMG_OPEN_KEY'] = 'Bouton d\'ouverture de la clé Symbiota';
$LANG['IMG_VOUCHERS_SIMPLE_MAP'] = 'Afficher les bons dans une carte simple';
$LANG['IMG_VOUCHERS_DYNAMIC_MAP'] = 'Afficher les bons dans la carte dynamique';
$LANG['IMG_OCCUR_DYNAMIC_MAP'] = 'Afficher toutes les occurrences dans la carte dynamique';
$LANG['EDIT_DETAILS'] = 'Bouton Modifier les détails';
$LANG['SEARCH_COMMON'] = 'Rechercher des noms communs';
$LANG['IMG_DWNL_LIST'] = 'Télécharger la liste';
$LANG['IMG_PRINT_LIST'] = 'Imprimer la liste';
$LANG['IMG_DOCX_EXPORT'] = 'Exporter vers DOCX';
$LANG['SKIP_LINK'] = 'Passer à la liste des taxons';
$LANG['IMAGE_SRC'] = 'Source de l\'image';
$LANG['ALL_IMG'] = 'toutes les images';
$LANG['LINKED_IMG'] = 'images de bons liés uniquement';
$LANG['SELECT_TAXON'] = 'Veuillez sélectionner un taxon dans la liste';
$LANG['NAME_BLANK'] = 'Le champ du nom scientifique est vide';

//from dynamicchecklist.php
$LANG['ERROR_GEN_CHECK'] = 'ERREUR lors de la génération de la liste';

?>
