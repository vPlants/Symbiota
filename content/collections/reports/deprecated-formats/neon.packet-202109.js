...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (labels) {
    labels.forEach((label) => {
      ...
      fetchTaxonomy(sciname).then(data => taxon.innerText = getVernacular(data.taxonomy) + ' of NEON');
      console.log(taxon);
      ...
      if (hasLocality && locality.innerText.includes('Domain')) {
        let domainNumber = label.querySelector('.locality').innerText.match(/(?<=Domain )(\d*)(?=,)/)[0];
        let domainNumberDiv = document.createElement('span');
        domainNumberDiv.innerText = ' Domain ' + domainNumber;
        let domainName = (domainsArr.filter(d => d.domainnumber === 'D' + domainNumber))[0].domainname;
      ...
      cArr.forEach((catNum) => {
        ...
        newCatNum += `<span>${catNum.trim()}</span>`;
      });
